...
interface TypeVerifyCodeRegisterProps {
  dialogOpen: (open: boolean) => void;
}
const TypeVerifyCodeRegister: React.FC<TypeVerifyCodeRegisterProps> = ({
  dialogOpen,
}) => {
  ...
  const { mutate: ResendMutate } = UseMutate({
    ...
    onSuccess: (responseData) => {
      // @ts-ignore
      router.replace(`${locale == "ar" ? "" : "/en"}/auth/verify-code-phone`);
      dialogOpen(false);
    },
...
export default TypeVerifyCodeRegister;
