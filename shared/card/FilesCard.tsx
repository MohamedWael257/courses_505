...
export default function FilesCard({
  ...
  const [isFavouriteState, setIsFavouriteState] = useState<boolean>(() => {
    if (favItems.length > 0) {
      return favItems.some((item: any) => item.id == fileData?.id);
    } else if (!favItems.length && !fileData?.is_favorite) {
      return false;
    }
  });
  useEffect(() => {
    if (favItems.length > 0) {
      setIsFavouriteState(
        favItems.some((item: any) => item.id == fileData?.id)
      );
    } else if (!favItems.length && !fileData?.is_favorite) {
      setIsFavouriteState(false);
    }
  }, [favItems, fileData?.id]);
  ...
  return (
    ...
    >
      ...
      <LocalePath href={`/files/${fileData?.id}`}>
      ...
      <div className="absolute top-6 start-7 text-2xl font-bold flex gap-3">
        <Heart
          onClick={() => handleFavouriteToggle(fileData?.id)}
      ...
      <div className="flex flex-col gap-2">
        <p className="text-base font-medium leading-4 text-start text-error">
          {fileData?.brand?.name || "المهارات المهنية والأعمال"}
        </p>
